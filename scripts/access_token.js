...
async function getToken(page, creds) {
    const authorizationPageUrl = `https://www.linkedin.com/oauth/v2/authorization?response_type=code&client_id=${creds.client_id}&redirect_uri=${creds.redirect_uri}&state=hey`;

    // go to the authorization page
    await page.goto(authorizationPageUrl);
    if(await page.$('#session_key-login')) { // need to log in
    ...
    if(!url.startsWith(creds.redirect_uri)) {
        ...
        if(!allowButton)
            throw Error('The authorization button is not present on the page.');
        ...
        url = await page.url();
        if(!url.startsWith(creds.redirect_uri))
            throw Error('Something went wrong during the redirection process.');
    }

    // get the code
    const code = url.match(/\?code=([^&]+)/)[1];
    if(!code)
        throw Error('No code has been found into the GET parameters.');
    ...
    if(!accessToken)
        throw Error('The request has failed, no access token has been found in the response.');
