...
module.exports = class LinkedinClient {
  constructor(cookie) {
    if (!cookie) throw new Error('The Linkedin cookie is required.')
  ...
  async fetch(url) {
    ...
    if (process.env.NODE_ENV == 'dev') fs.writeFileSync(debugFile, '')

    const res = await request(url, { headers: { Cookie: `li_at=${this.cookie}` } })
    const html = await res.text()
    const $ = cheerio.load(html)
    let data,
      result = { linkedinUrl: url.replace('/about/', ''), related: [] }
    while (!result.name && !result.firstName) {
      ...
      for (let elt of $('code').get()) {
        ...
        for (let item of data.included) {
          ...
          if (process.env.NODE_ENV == 'dev') fs.appendFileSync(debugFile, JSON.stringify(item, null, 4) + '\n')
        }
      }

      // this company or people does not exist
      if (!result.firstName && !result.name) return null
    }
...
function processPeopleProfile(item, result) {
  if (item.$type == 'com.linkedin.voyager.dash.common.Industry' && item.name) industries[item.entityUrn] = item.name
...
function processCompanyPage(item, result) {
  if (item.$type == 'com.linkedin.voyager.common.Industry') {
    ...
    result.companySize =
      item.staffCountRange.start + (item.staffCountRange.end ? '-' + item.staffCountRange.end : '+') + ' employees'
